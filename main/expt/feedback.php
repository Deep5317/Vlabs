    ...
    <form name="preTest" method="POST">
        ...
        <?php if ($_SESSION['logged_in'] == 'true') { ?>
            <div class="form-floating">
                <textarea class="form-control" name="comments" placeholder="Leave a comment here" id="floatingTextarea" required></textarea>
            </div>
...
if (isset($_REQUEST["feedback"])) {
    ...
    if ($_SESSION['logged_in'] == 'true') {
        ...
        if ($count == 0) {
            $sql7 = "INSERT INTO `fbcomment`(`subID`, `expID`, `userID`, `comments`) VALUES ('$SubID', '$ExpID','$userID','$comment')";
            mysqli_query($db, $sql7);
        } else {
            $sql7 = "UPDATE `fbcomment` SET `comments` = '$comment' WHERE `subID`= $SubID && `expID`=$ExpID && `userID`=$userID";
            mysqli_query($db, $sql7);
        }
        ...
        foreach ($_POST as $ques_no => $ANS) {
            ...
            $sql = "INSERT INTO `fbresponse` (`subID`, `expID`, `feedbackID`, `answer`) VALUES ('$SubID', '$ExpID', '$ques_no', '$ANS');";
            mysqli_query($db, $sql);
